Training a 3DGS scene starts with a point cloud which we take as the positions for initial 3D Gaussians. After that forward and backpropagation steps alternate similar to those in neural networks \cite{thimm2025ML}, modifying opacity, position, shape and color of each 3D Gaussian in the scene. After each error-correcting backpropagation step the forward step renders an image from the same camera pose as the corresponding original image until differences between rendered and original images converge which takes tens of thousands of iterations.
...
Furthermore, another paper in this seminar deals with neural rendering.
...
3DGS as originally conceived uses these points as the origins for the initial 3D Gaussians which are being initialized as perfect spheres having radii deduced from the three neigh\-boring spheres with an opacity of around $0.1$. The initial color can be any RGB value.

Consequently, having to derive camera poses for initialization first adds significantly to the entire training process which is why other, faster COLMAP-free methods have been proposed, see chapter \ref{ch:improvements}. Also, Kerbl et al \cite{kerbl3DGaussianSplatting2023} mention that using random initialization of all 3D Gaussians yields good quality results despite longer training times.
