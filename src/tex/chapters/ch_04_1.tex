Training a 3DGS scene starts with a point cloud which we take as the positions for initial 3D Gaussians. Following, forward and backpropagation passes alternate similar to those in neural networks~\cite{thimm2025ML}, modifying opacity, position, shape and color of each 3D Gaussian in the scene. After each error-correcting backward pass the forward pass renders an image from the same camera pose as the corresponding input image until differences between rendered and input images converge which takes tens of thousands of iterations.
