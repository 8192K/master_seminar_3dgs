...
\begin{itemize}
    ...
    \item Covariance matrix $\Sigma$: A $3\times 3$ matrix determining 3D shape and rotation of the 3D Gaussian, but decomposed into three scaling values and four rotational values (\textit{quaternion}). Decomposing the covariance matrix is pivotal for the backpropagation process, see chapter \ref{ch:matrix_decomp}. (7 attributes)
    \item Spherical harmonics coefficients: model view-dependent colors and specular reflections. Spherical harmonics come in four degrees \cite{plenoctrees}. SH0 is a simple RGB value denoting a solid color of the 3D Gaussian that is not dependent on viewing angles. Degrees SH1 to SH3 contribute 3, 5 and 7 attributes per color channel each,  adding more and more to realism. SH3 is recommended for 3DGS  \cite{kerbl3DGaussianSplatting2023}. For the remainder of this paper we will disregard spherical harmonics and their complexity and implicitely mean SH when referring to any notion of ''color''. (3-48 attributes)
\end{itemize}
